    ...
    <div class="row">
        ...
        <div class="col-center">
            ...
            <div class="contact-form">
                <h2>Contact Form</h2>
                <form id="contactForm" action="<?= $link->url('home.save') ?>" method="post">
                    <input type="hidden" name="id" value="<?= @$data['contact']?->getId() ?>">

                    <div class="form-group">
                        <label for="name">Your Name:</label>
                        <input type="text" id="name" name="name" required value="<?= @$data['contact']?->getName() ?>">
                    </div>

                    <div class="form-group">
                        <label for="email">Your Email:</label>
                        <input type="email" id="email" name="email" required value="<?= @$data['contact']?->getEmail() ?>">
                    </div>

                    <div class="form-group">
                        <label for="subject">Subject:</label>
                        <input type="text" id="subject" name="subject" required value="<?= @$data['contact']?->getSubject() ?>">
                    </div>

                    <div class="form-group">
                        <label for="message">Message:</label>
                        <textarea id="message" name="message" rows="4" required><?= @$data['contact']?->getMessage() ?></textarea>
                    </div>
